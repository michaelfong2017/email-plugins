...
# Init db
def init_db(conn, logger=None):
    start_time = datetime.datetime.now()
    conn.execute('''CREATE TABLE IF NOT EXISTS known_sender (address TEXT PRIMARY KEY NOT NULL);''')
    conn.execute('''CREATE TABLE IF NOT EXISTS junk_sender (address TEXT PRIMARY KEY NOT NULL);''')
    conn.commit()
...
def is_address_exists_in_junk_sender(address, conn, logger=None):
    try:
        cursor = conn.execute(f'''SELECT count(*) FROM junk_sender WHERE address = {address};''')
        records = cursor.fetchall()
        match_count = records[0][0]
